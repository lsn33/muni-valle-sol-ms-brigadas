 ...
 * ({@link cl.municipalidad.msbrigadas.controller.BrigadaController}) y el repositorio
 * ({@link BrigadaRepository}), encapsulando todas las reglas de negocio.</p>
 *
 * <p>Usa {@link BrigadaFactory} para crear brigadas siguiendo el patrón
 * Factory Method, y {@code @Transactional} para garantizar la integridad
 * de los datos en operaciones de escritura.</p>
 *
 * @author Municipalidad Valle del Sol
 * @version 1.0
 * @see BrigadaRepository
 * @see BrigadaFactory
 */
@Service
...
public class BrigadaService {
    ...
    private final BrigadaFactory brigadaFactory;

    /**
     * Registra una nueva brigada en el sistema.
     *
     * <p>Delega la creación del objeto a {@link BrigadaFactory} para aplicar
     * las reglas de negocio según el tipo de brigada.</p>
     *
     * @param request DTO validado con los datos de la nueva brigada.
     * @return {@link BrigadaDTO} con los datos de la brigada creada.
     * @throws IllegalArgumentException si el tipo de brigada no es reconocido.
     */
    @Transactional
    public BrigadaDTO crear(CreateBrigadaRequest request) {
     ...
     *
     * @return Lista de {@link BrigadaDTO}. Vacía si no hay brigadas registradas.
     */
    @Transactional(readOnly = true)
    public List<BrigadaDTO> listarTodas() {
     ...
     *
     * @return Lista de {@link BrigadaDTO} con brigadas disponibles.
     */
    @Transactional(readOnly = true)
    public List<BrigadaDTO> listarDisponibles() {
     ...
     * @return Lista de {@link BrigadaDTO} del tipo indicado.
     */
    @Transactional(readOnly = true)
    public List<BrigadaDTO> listarPorTipo(String tipo) {
     ...
     * @return {@link BrigadaDTO} con los datos de la brigada.
     * @throws RuntimeException si no existe una brigada con ese ID.
     */
    @Transactional(readOnly = true)
    public BrigadaDTO buscarPorId(Long id) {
     ...
     * @return {@link BrigadaDTO} con el estado actualizado.
     * @throws RuntimeException si no existe una brigada con ese ID.
     */
    @Transactional
    public BrigadaDTO actualizarEstado(Long id, UpdateEstadoRequest request) {
     ...
     * @return {@link BrigadaDTO} con la ubicación actualizada.
     * @throws RuntimeException si no existe una brigada con ese ID.
     */
    @Transactional
    public BrigadaDTO actualizarUbicacion(Long id, UpdateUbicacionRequest request) {
        Brigada brigada = brigadaRepository.findById(id)
                .orElseThrow(() -> new RuntimeException("Brigada no encontrada con id: " + id));
    ...
    /**
     * Convierte una entidad {@link Brigada} a su representación pública {@link BrigadaDTO}.
     *
     ...
     * @return {@link BrigadaDTO} con los datos públicos de la brigada.
     */
    private BrigadaDTO toDTO(Brigada brigada) {
        return new BrigadaDTO(
            brigada.getId(),
            brigada.getNombre(),
            brigada.getEstado(),
            brigada.getTipo(),
